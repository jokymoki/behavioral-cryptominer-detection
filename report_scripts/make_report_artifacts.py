...
def get_feature_columns(score_data: dict) -> list[str]:
    if "feature_columns" not in score_data:
        raise KeyError("Score file does not contain feature_columns")
    return list(score_data["feature_columns"])
...
def build_feature_importance_table(
    ...
    weighted_sum = None
    weighted_count = None
    abs_weighted_sum = None
    feature_columns = None
    per_file_rows = []

    stride = results["window_config"]["stride"]
    true_interval = results.get("true_infected_interval_sec")

    for item in results["files"]:
        data = torch.load(scores_path(item["scores_path"]), map_location="cpu", weights_only=False)
        z_pos = to_numpy(data["z_feature_pos"])
        feature_mse = to_numpy(data["feature_mse"])

        if feature_columns is None:
            feature_columns = get_feature_columns(data)

        if mode == "infected":
            mask = np.ones(z_pos.shape[0], dtype=bool)
        elif mode == "mixed_infected_interval":
            if true_interval is None:
                raise ValueError("mixed_infected_interval mode requires true_infected_interval_sec")
            starts = np.arange(z_pos.shape[0]) * stride
            # Importance follows the same labeling convention as score_mixed_dataset:
            # a window is infected if its [start, start + T + H] span overlaps the infected interval.
            window_span = results["window_config"]["T"] + results["window_config"]["H"]
            ends = starts + window_span
            mask = (starts < true_interval["end"]) & (ends > true_interval["start"])
        elif mode == "mixed_positive_windows":
            score = to_numpy(data[results["score_key"]])
            mask = score > results["threshold"]
        else:
            raise ValueError(f"Unknown feature importance mode: {mode}")

        if not mask.any():
            continue

        selected_z = z_pos[mask]
        selected_mse = feature_mse[mask]
        file_mean = selected_z.mean(axis=0)
        file_abs_mse = selected_mse.mean(axis=0)

        if weighted_sum is None:
            weighted_sum = np.zeros_like(file_mean, dtype=np.float64)
            abs_weighted_sum = np.zeros_like(file_abs_mse, dtype=np.float64)
            weighted_count = 0

        weighted_sum += selected_z.sum(axis=0)
        abs_weighted_sum += selected_mse.sum(axis=0)
        weighted_count += selected_z.shape[0]
            ...
            per_file_rows.append(
                {
                    "file": short_name(item["file"]),
                    "feature": feature,
                    "windows_used": int(selected_z.shape[0]),
                    "mean_positive_z": float(file_mean[idx]),
                    "mean_feature_mse": float(file_abs_mse[idx]),
                }
            )

    if weighted_count == 0 or feature_columns is None:
        return pd.DataFrame()

    mean_z = weighted_sum / weighted_count
    mean_mse = abs_weighted_sum / weighted_count
    total = mean_z.sum()
    importance_share = mean_z / total if total > 0 else np.zeros_like(mean_z)
        ...
        rows.append(
            {
                "rank": 0,
                "feature": feature,
                "importance": float(mean_z[idx]),
                "importance_share": float(importance_share[idx]),
                "mean_feature_mse": float(mean_mse[idx]),
                "windows_used": int(weighted_count),
            }
        )
...
def save_feature_importance_tables(
    infected_results: dict,
    mixed_results: dict,
) -> tuple[list[Path], dict[str, pd.DataFrame]]:
    outputs = []
    tables = {
        "feature_importance_infected": build_feature_importance_table(infected_results, mode="infected"),
        "feature_importance_mixed_infected_interval": build_feature_importance_table(
            mixed_results,
            mode="mixed_infected_interval",
        ),
        "feature_importance_mixed_positive_windows": build_feature_importance_table(
            mixed_results,
            mode="mixed_positive_windows",
        ),
    }
...
def plot_feature_importance(df: pd.DataFrame, name: str, top_n: int = 15) -> Path:
    top = df.head(top_n).iloc[::-1]
    out_path = FIG_DIR / f"{name}_top{top_n}.png"

    plt.figure(figsize=(9, 6))
    plt.barh(top["feature"], top["importance"])
    plt.xlabel("mean positive standardized error")
...
def save_feature_importance_figures(tables: dict[str, pd.DataFrame]) -> list[Path]:
    outputs = []
    for name, df in tables.items():
        if not df.empty:
            outputs.append(plot_feature_importance(df, name, top_n=15))
    return outputs
...
def main() -> None:
    ...
    feature_table_outputs, feature_tables = save_feature_importance_tables(infected_results, mixed_results)
    table_outputs.extend(feature_table_outputs)
    figure_outputs = save_figures(threshold, infected_results, mixed_results, mixed_df)
    figure_outputs.extend(save_feature_importance_figures(feature_tables))
